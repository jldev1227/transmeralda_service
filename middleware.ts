...
export async function middleware(request: NextRequest) {
  ...
  if (permissionConfig && permissionConfig.permissions.length > 0) {
    console.log(`[Middleware] Ruta protegida: ${path}, requiere permisos:`, permissionConfig.permissions);
    ...
    if (!userInfoCookie || !tokenCookie) {
      ...
      return redirectToLoginWithError(url, 'Debe iniciar sesión para acceder a esta página');
    }
    
    try {
      // Parsear la información del usuario
      const userInfo = JSON.parse(userInfoCookie) as User;
      console.log(`[Middleware] Usuario: ${userInfo.nombre}, Rol: ${userInfo.role}`);
      
      // Verificar si el usuario tiene los permisos necesarios
      const hasPermission = checkPermissions(userInfo, permissionConfig.permissions);
      console.log(`[Middleware] ¿Tiene permisos? ${hasPermission ? 'SÍ' : 'NO'}`);
      ...
      requestHeaders.set('x-user-permissions', JSON.stringify(userInfo.permisos || {}));
      ...
      return redirectToLoginWithError(url, 'Error en la sesión. Por favor, inicie sesión nuevamente');
    }
  } else {
...
function getRequiredPermissions(path: string): PermissionConfig | null {
  ...
  const routes = Object.keys(PROTECTED_ROUTES).sort((a, b) => b.length - a.length);
...
function checkPermissions(userInfo: User, requiredPermissions: string[]): boolean {
  console.log("Verificando permisos:", userInfo.role, "Permisos requeridos:", requiredPermissions);
...
function redirectToLoginWithError(url: URL, errorMessage: string): NextResponse {
...
function redirectToError(url: URL, errorMessage: string): NextResponse {
  console.log(`[Middleware] Redirigiendo a error page con mensaje: ${errorMessage}`);
