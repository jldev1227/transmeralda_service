...
export async function POST(request: NextRequest) {
  try {
    ...
    if ((!token && service !== "token/login") || !service || !params) {
        return NextResponse.json({ error: 'Faltan parámetros requeridos: token, service, params' }, { status: 400 });
    }
  ...
  } catch (error: any) { // Es buena práctica tipar el error si es posible, o usar 'any'/'unknown'
