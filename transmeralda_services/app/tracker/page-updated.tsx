...
const getServiceTypeText = (type: string): string => {
  switch (type.toLowerCase()) {
    case "pasajeros": return "Transporte de Pasajeros";
    case "carga": return "Transporte de Carga";
...
const TrackerPage = () => {
    ...
    async (sessionIdOrToken: string, endpoint: string, params: any) => {
      ...
      try {
        ...
        if (response.data && response.data.error) {
          throw new Error(`Error Wialon API (${response.data.error}): ${response.data.reason || endpoint}`);
        }

        return response.data;
      } catch (err) {
        console.error(`Error llamando a ${endpoint} via /api/wialon-api:`, err);
        setError(`Error al comunicarse con Wialon: ${err instanceof Error ? err.message : "Desconocido"}`);
        throw err;
      }
    },
    []
  ...
  const handleServicioClick = useCallback((servicio) => {
    if (selectServicio) {
    ...
    return (
      <div className="p-6 text-center">
        <div className="bg-red-100 p-4 rounded-md text-red-800 shadow-md">
          ...
          <p>No se encontró el token de Mapbox. Por favor configure la variable de entorno NEXT_PUBLIC_MAPBOX_TOKEN.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="h-[calc(100vh-3.5rem)] p-4 bg-gray-50">
      <div className="h-full flex flex-col">
        <div className="mb-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-800">Rastreador de Servicios</h1>
          {error && (
