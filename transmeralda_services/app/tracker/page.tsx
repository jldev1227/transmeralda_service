...
import React, { useState, useEffect, useCallback } from "react"; // Import useMemo
...
const WialonVehicles = () => {
  ...
  useEffect(() => {
    ...
    const fetchInitialAndDetails = async () => {
      ...
      try {
        ...
        if (!loginData?.eid)
          throw new Error("Login fallido: No se obtuvo Session ID (eid)");
        sessionId = loginData.eid;

        // 2. Lista Inicial
        const initialVehicleData = await callWialonApi(
          sessionId,
          "core/search_items",
          {
            spec: {
        ...
        if (
          !initialVehicleData?.items ||
          !Array.isArray(initialVehicleData.items) ||
          initialVehicleData.items.length === 0
        ) {
          if (isMounted) setLoading({ initial: false, details: false });

          return;
        }
        const initialVehicles: WialonVehicle[] = initialVehicleData.items.map(
          (item: any) => ({
            id: item.id,
            nm: item.nm,
            cls: item.cls,
        ...
        const detailPromises = initialVehicles.map((vehicle) =>
          callWialonApi(sessionId!, "core/search_item", {
            id: vehicle.id,
            flags: 1025,
          }) // Usando flag 1025
            .then((itemData) => ({
        ...
        setVehicles((prevVehicles) => {
          const updatedVehicles = prevVehicles.map((vehicle) => {
            ...
            if (result?.status === "fulfilled" && result.data) {
              ...
              return {
                id: vehicle.id,
                nm: result.data.nm ?? vehicle.nm,
                cls: result.data.cls ?? vehicle.cls,
                pos: result.data.pos,
                lmsg: result.data.lmsg,
                detailsLoaded: true,
                errorLoadingDetails: undefined,
                simulatedState,
              };
              ...
              return {
                ...vehicle,
                detailsLoaded: false,
                errorLoadingDetails: result.reason,
              };
            }
      ...
      if (!flyToTarget) {
        ...
        if (vehiclesWithPos.length > 0) {
          const bounds = L.latLngBounds(
            vehiclesWithPos.map((v) => [v.pos!.y, v.pos!.x]),
          );
  ...
  const handleVehicleCardClick = (vehicle: WialonVehicle) => {
    if (vehicle.pos?.y && vehicle.pos?.x) {
      ...
      setFlyToTarget({
        coords: [vehicle.pos.y, vehicle.pos.x],
      ...
      const marker = document.querySelector(
        `.leaflet-marker-icon[data-vehicle-id="${vehicle.id}"]`,
      );

      if (marker instanceof HTMLElement && (marker as any)._leaflet_pos) {
        // Verificar si es un marcador leaflet
  ...
  const vehiclesWithPosition = vehicles.filter(
    (v) => v.pos?.y && v.pos?.x && v.detailsLoaded,
  );
  ...
  return (
    ...
    >
      <h1 style={{ textAlign: "center", margin: 0, color: "#374151" }}>
        Mapa de Vehículos Wialon (Yopal)
      </h1>
      {isLoading && "Cargando..."}
      {error && (
        <div
          className="error-message p-2 border rounded bg-red-50 border-red-200 text-center"
          style={{ color: "red" }}
        >
          ⚠️ Error: {error}
        </div>
        ...
        {vehicles.map((vehicle) => (
          <div
            ...
            style={{
              ...
              backgroundColor:
                activeVehicleId === vehicle.id ? "#eff6ff" : "white", // Resaltar si está activa
          ...
          >
            ...
            <div style={{ fontSize: "12px", color: "#4b5563" }}>
              {vehicle.pos ? (
                <>
                  <div>
                    Estado: {getSimulatedStatusText(vehicle.simulatedState)}
                  </div>
                  <div>
                    Vel:{" "}
                    {typeof vehicle.pos.s === "number"
                      ? `${vehicle.pos.s} km/h`
                      : "N/D"}
                  </div>
      ...
      <div
        ...
        style={{
          /* ... (estilos del contenedor sin cambios) ... */ flexGrow: 1,
          width: "100%",
          borderRadius: "8px",
          overflow: "hidden",
      ...
      >
        ...
        >
          ...
          <MapViewController flyToTarget={flyToTarget} vehicles={vehicles} />
          ...
          {vehiclesWithPosition.map((vehicle) => (
            ...
            >
              <Popup className="vehicle-marker-popup">
                ...
                <div>
                  <div
                    className={`popup-header ${getPopupHeaderClass(vehicle.simulatedState)}`}
                  >
                    {vehicle.nm || "N/D"}
                  </div>
                  <div className="popup-content">
                    ...
                    <div>
                      Lat: {vehicle.pos!.y.toFixed(5)}, Lng:{" "}
                      {vehicle.pos!.x.toFixed(5)}
                    </div>
                    ...
                    >
                      <div>
                        <div className="popup-label">Velocidad</div>
                        <div>
                          {typeof vehicle.pos!.s === "number"
                            ? `${vehicle.pos.s} km/h`
                            : "N/D"}
                      ...
                      <div>
                        <div className="popup-label">Estado</div>
                        <div>
                          {getSimulatedStatusText(vehicle.simulatedState)}
                        </div>
                      </div>
                    </div>
                    {vehicle.pos!.t && (
                      <>
                        {" "}
                        <div className="popup-divider" />{" "}
                        <div className="popup-label">Última Actualización</div>{" "}
                        <div>
                          {new Date(vehicle.pos!.t * 1000).toLocaleString(
                            "es-CO",
                    ...
                    {vehicle.errorLoadingDetails && (
                      <>
                        ...
                        <div style={{ color: "orange", fontSize: "12px" }}>
                          (Error: {vehicle.errorLoadingDetails})
                        </div>{" "}
                      </>
                    )}
                  </div>
      ...
      >
        Mostrando {vehiclesWithPosition.length} de {vehicles.length} vehículos
        con posición. {isLoading && " Actualizando..."}
      </div>
    </div>
  );
};
