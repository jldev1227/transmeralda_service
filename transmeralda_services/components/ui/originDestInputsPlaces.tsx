...
export default function SearchInputsPlaces() {
    ...
    const [destCoordinates, setDestCoordinates] = useState<{ lat: number, lng: number } | null>(null);
    ...
    // Aplicar debounce
    const debouncedFetchOrigin = useRef(debounce((value: string) => fetchPredictions(value, true), 300)).current;
    const debouncedFetchDest = useRef(debounce((value: string) => fetchPredictions(value, false), 300)).current;
    ...
    const selectPrediction = async (prediction: Prediction, isOrigin: boolean) => {
        if (isOrigin) {
            ...
            if (details?.location) {
                setOriginCoordinates(details.location);
            ...
            <div className="relative" ref={originRef}>
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        onFocus={() => originSpecific.length >= 3 && setShowOriginPredictions(true)}
                    />
                    {loadingOrigin && (
            ...
            <div className="relative" ref={destRef}>
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        value={destSpecific}
                        onFocus={() => destSpecific.length >= 3 && setShowDestPredictions(true)}
                    />
                    {loadingDest && (
                        <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                            <svg className="animate-spin h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
