...
export const useWialonRealtime = ({
    ...
    try {
      ...
      if (response.data && !response.data.error) {
        ...
        if (response.data && response.data.error === 2) {
          throw new Error(
            "Token de sesión inválido o expirado (ACCESS_DENIED)",
          );
        }
        throw new Error(
          response.data?.error
      ...
      setError(
        err instanceof Error
          ? err
          : new Error("Error desconocido en la suscripción"),
      );
  ...
  const pollUpdates = useCallback(async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        if (response.data.error) {
          if (response.data.error === 2) {
            throw new Error(
              "Token de sesión inválido o expirado durante el polling",
            );
          }
      ...
      setError(
        err instanceof Error ? err : new Error("Error desconocido en polling"),
      );
      setIsConnected(false);
      if (onError) onError(err);
    } finally {
